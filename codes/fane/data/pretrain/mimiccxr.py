...
from fane.data.pretrain.base import PretrainDataset
from fane.data.pretrain.text_process import SentenceSplitter, split_into_sections
from fane.transoforms.language import SentenceShuffle
...
class MimicCxrDataset(PretrainDataset):
    def __init__(self, dataset_path, num_colors=1, image_transform=[], text_transform=[], rate=1.0,  max_length=112):
        self.rate = rate
        self.num_colors = num_colors
        self.filter = filter
        super().__init__(dataset_path, image_transform=image_transform, text_transform=text_transform, rate=rate)
        self.tokenizer = AutoTokenizer.from_pretrained("/Bio_ClinicalBERT")
...
class MimicCxrDatasetBuilder():
    ...
    def build(self):
        ...
        print('size: ', len(data_json))
        with open(os.path.join(self.save_path, 'data_copy.json'), 'w') as f:
...
'''
@click.command('build')
@click.option('--root_image_path', default='/data/***/mimic-cxr-images/', help='directory of MIMIC-CXR image dataset')
@click.option('--root_text_path', default='/data/***/mimic-cxr-reports/files/', help='directory of MIMIC-CXR image dataset')
@click.option('--meta_csv_path', default='/data/***/mimic-cxr-2.0.0-metadata.csv', help='path of meta csv file')
@click.option('--save_path', default='/data/***/', help='path of saved file')
