...
from fane.data.downstream.detection_dataset import (OBJCXRDetectionDataset,
                                             RSNADetectionDataset)

from fane.data.downstream.transforms import DetectionDataTransforms
from fane.encoders.image.detector_backbone import ResNetDetector
from fane.downstream.ssl_detector import SSLDetector
...
def cli_main():
    ...
    parser.add_argument("--ckpt_path", type=str, default="xxxxxx.ckpt")
    parser.add_argument("--dataset", type=str, default="rsna", help="rsna or object_cxr")
    ...
    ckpt_dir = os.path.join('/data', f"HAMeR/detection/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.95")
    os.makedirs(ckpt_dir, exist_ok=True)

    logger_dir = os.path.join('/data', f"HAMeR/detection/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.95/logs")
    os.makedirs(logger_dir, exist_ok=True)

    file_path = Path(os.path.join('/data', f"HAMeR/detection/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.95/training.log"))
