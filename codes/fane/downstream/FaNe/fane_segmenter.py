...
from fane.downstream.ssl_segmenter import SSLSegmenter
from fane.models.mcga import Mcga

from fane.data.downstream.data_module import DataModule
from fane.data.downstream.segmentation_dataset import RSNASegmentDataset, SIIMImageDataset
from fane.encoders.image.transformer_seg import SETRModel
...
def cli_main():
    ...
    if args.dataset == "siim":
        datamodule = DataModule(SIIMImageDataset, None,
                                None, args.data_pct,
                                args.batch_size, args.num_workers, crop_size=224)
    ...
    if args.base_model == 'resnet50':
        ...
        image_encoder = VitEncoder(model_name='vit_base', output_dim=128, image_size=224, weights_path = "xxxx.pth")
        text_encoder = ClinicalBERT()
    ...
    mgca = Mcga.load_from_checkpoint(args.ckpt_path,
                                     ...
                                     gpus=[0],
                                     strict=False)
    encoder = mgca.image_encoder.model
    ...
    ckpt_dir = os.path.join('/data', f"HAMeR/segmentation/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.6")
    os.makedirs(ckpt_dir, exist_ok=True)
    logger_dir = os.path.join('/data', f"HAMeR/segmentation/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.6/logs")
    os.makedirs(logger_dir, exist_ok=True)
    file_path = Path(os.path.join('/data', f"HAMeR/segmentation/{args.base_model}/{args.dataset}_{args.data_pct}_{extension}_full_loss_threshold_0.6/training.log"))
