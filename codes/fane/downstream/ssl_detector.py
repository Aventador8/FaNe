...
from fane.utils.yolo_loss import YOLOLoss
from fane.data.downstream.data_module import DataModule
from fane.data.downstream.transforms import DataTransforms

from torchmetrics.detection.mean_ap import MeanAveragePrecision
from fane.utils.detection_utils import non_max_suppression
...
class SSLDetector(LightningModule):
    ...
    def validation_epoch_end(self, validation_step_outputs):
        torch.use_deterministic_algorithms(False)  # 临时禁用
        map = self.val_map.compute()["map"]
        torch.use_deterministic_algorithms(True)  # 恢复设置
    ...
    def num_training_steps(trainer, dm) -> int:
        ...
        num_processes = getattr(trainer, "world_size", 1)
        num_devices = max(1, num_devices, num_processes)
...
if __name__ == "__main__":
    from fane.data.downstrean.detection_dataset import RSNADetectionDataset
    model = ModelMain()
