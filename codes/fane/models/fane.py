...
class Fane(PretrainModel):
    task = 'pretrain'

    def __init__(self, text_encoder, image_encoder, gpus, num_heads = 1, threshold=0.95, stage1_epochs=20, stage2_epochs=30, stage3_epochs=50,
                 ...
                 temp_decay='fixed', frozen_text_encoder=False, ckpt_path='checkpoints/', image_encoder_mode = 'resnet50', down_stream = False):
        ...
        self.lambda_intra =  1.0         #   3.0
        self.lambda_local_text = 1.0   # 0.3
        self.lambda_regular = 1.0       # 0.05

        # self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / temperature))
        # self.local_logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / local_temperature))
        # self.intra_logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / local_temperature))

        if image_encoder_mode == 'resnet50':
            self.local_vision_width = 1024
            self.global_vision_width = 2048
        else:
            self.local_vision_width = 768
            self.global_vision_width = 768
        ...
        self.global_image_projection = nn.Linear(self.global_vision_width, self.embed_dim)
        ...
        # self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / temperature))
        # self.local_logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / local_temperature))
        # self.intra_logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / intra_temperature))
        ...
        if not down_stream:
            self.train_iters_per_epoch = len(self.train_dataset) // (len(gpus) * batch_size)      # finetune 时 注释掉
    ...
    def inititalize_parameters(self):
        ...
        nn.init.normal_(self.global_image_projection.weight, std=self.global_vision_width ** -0.5)
    ...
    def encode_image(self, image):
        ...
        # print(f"global_image_features shape: {global_image_features.shape}")
    ...
    def getTextSoftTarget(self, raw_txt, threshold):
        ...
        with torch.no_grad():
            ...
            scores.fill_diagonal_(threshold + margin)
    ...
    def global_alignment_and_intra_modality_loss(self, f_img, f_txt, labels):
        ...
        for layer, score in enumerate(scores):
            ...
            loss_text = torch.sum(neg_text_weighted)
            loss_image = torch.sum(neg_image_weighted)

            intra_loss += (loss_text + loss_image) / 2


        M = torch.ones((batch_size,batch_size), device=f_img.device) * -1
        mask = scores > threshold
        M[mask] = 1
        # Calculate the argument for the softplus function:
        # A_ij = m_ij * (-s_ij / tau + beta)
        A = M * (-logits_i2t * self.tau + self.beta)

        # Calculate softplus for each element
        # softplus(x) = log(1 + exp(x))
        element_wise_terms = F.softplus(A)

        # Sum all element-wise terms
        sum_of_terms = torch.sum(element_wise_terms)

        global_alignment_loss = sum_of_terms / batch_size
    ...
    def local_alignment_loss(self, local_image_embed_stacks, local_text_embed_stacks):
        """
        local_image_embed_stacks: (B, I, D)
        local_text_embed_stacks: list of B tensors with shape (T_i, D)
        """
        # total_image_loss = 0.
        total_text_loss = 0.
        total_regular = 0.
        batch_size = local_image_embed_stacks.size(0)
        # TODO: maybe we can optimize this step ?
        # get each instance
        for idx in range(batch_size):
            local_text_embed = local_text_embed_stacks[idx]  # get sentence-level representation    ( T D )
            local_image_embed = local_image_embed_stacks[idx]  # get patch-level representation     ( I D )

            # print(f"local_image_embed shape: {local_image_embed.shape}")
            # print(f"local_text_embed shape: {local_text_embed.shape}")

            text_to_local_image_embed, _, regular_t2i = self.local_cross_attention(local_image_embed, local_text_embed, sparse=True)
            # text_to_local_image_embed, _ = self.local_cross_attention(local_image_embed, local_text_embed, sparse=True)                 # no sparse

            # local image to text loss
            text_loss = self.local_loss_fn(local_text_embed, text_to_local_image_embed)

            # # local text to image loss
            # image_loss = self.local_loss_fn(local_image_embed, text_to_local_image_embed)

            # total_image_loss += image_loss
            total_text_loss += text_loss
            total_regular += regular_t2i

        return {'sparse_regular_loss': total_regular / batch_size,
                'local_align_loss': total_text_loss / batch_size}
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Embed text
        local_text_embed = self.encode_text(text)     # 98 * 112 * 128      bs * length * 128
